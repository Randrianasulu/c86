...
#else
                fatal("failed to execute command", NULL, 0);
#endif
            else if (signore)
...
int rcsstat (char * name, struct stat * info)
{
  size_t len;
  char newname[100];
...
char        **pinputname;
{
  ...
  *p = '\0';
  if ((*pbasename = malloc((size_t)strlen(str2)+1)) == (char *)0 )
