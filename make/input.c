...
void init()
{
  ...
  if ((str1 = malloc((size_t)LZ1)) == (char *)0)
     fatal("No memory for str1",(char *)0,0);
  str1s.ptr = &str1;
  str1s.len = LZ1;
  if ((str2 = malloc((size_t)LZ2)) == (char *)0)
...
{
  strs->len *= 2;
  if( (*strs->ptr = realloc( *strs->ptr, (size_t)strs->len)) == (char *) NULL)
...
char *name;
{
  ...
  if ( (suff = suffix(name)) != (char *)NULL) {
     ...
     if (i > maxsuffarray) {
        ...
        if ((cp = malloc((size_t)strlen(suff)+1)) == (char *)0)
           fatal("No memory for name",(char *)0,0);
        strcpy(cp, suff);
        (*sp)->n_name = cp;
     }
...
struct cmd *cp;
{
  ...
  if ((rcp = malloc((size_t)strlen(str)+1)) == (char *)0)
    fatal("No memory for command",(char *)0,0);
  strcpy(rcp, str);
  rp->c_cmd = rcp;
...
#else
  if ((np->n_name[0] == '.' || np->n_name[0] == '_') && !dp && !cp) {
#endif
...
#else
    if ((np->n_name[0] == '.' || np->n_name[0] == '_') && dp == (struct depend *)0) {
#endif
        np->n_line->l_cmd = cp;
        return;
    }
...
#else
    if ((*str1 == TABCHAR)  || (*str1 == ' '))	/*  Rules without targets  */
#endif
