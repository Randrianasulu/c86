...
#ifdef MINIX
# ifdef BSD_A_OUT
#  define FILEHEADERLENGTH 32 /* orig 48, changed for linux */
# else
...
typedef struct {
    ...
    UINT DylCount;
    UBYTE DylTableOffset[4];
    UINT Spare;
} ImgHeader;
extern void checksum(char*);
#endif
...
#ifdef EPOC_TARGET
		segsz[seg] = roundup(segsz[seg], 16, offset_t);
		comsz[seg] = roundup(comsz[seg], 16, offset_t);
#else
...
#ifdef EPOC_TARGET
	if ((size = roundup(segpos[seg], 16, offset_t) - segpos[seg]) != 0)
#else
...
PRIVATE void writeheader()
{
	...
	p[i] = 0;
    offtocn((char *)&header.ImageVersion, 0x200f, sizeof header.ImageVersion);
    offtocn((char *)&header.HeaderSizeBytes, sizeof header,
					 sizeof header.HeaderSizeBytes);
    offtocn((char *)&header.CodeParas, (etextpadoff-btextoffset+15)/16,
					 sizeof header.CodeParas);
    offtocn((char *)&header.StackParas, 0x0100, sizeof header.StackParas);
    offtocn((char *)&header.DataParas, (endoffset-bdataoffset+15)/16,
					 sizeof header.DataParas);
    offtocn((char *)&header.HeapParas, 0x0080, sizeof header.HeapParas);
    offtocn((char *)&header.InitializedData, endoffset - bdataoffset,
					 sizeof header.InitializedData);
    offtocn((char *)&header.CodeVersion, 0x100f, sizeof header.CodeVersion);
    offtocn((char *)&header.Priority, 0x80, sizeof header.Priority);
    writeout((char *) &header, sizeof header);
}
#endif /* EPOC_TARGET */
