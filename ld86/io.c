...
char *filename;
{
	...
	if ((infd = open(filename, O_RDONLY | O_BINARY)) < 0)
...
char *filename;
{
...
#else
    if ((outfd = open (filename, O_WRONLY | O_CREAT | O_TRUNC | O_BINARY , CREAT_PERMS)) == ERR)
#endif /* GENERIC */
    ...
    outbufptr = outbuf;
#ifdef BSD_A_OUT
    if ((trelfd = open(filename, O_WRONLY | O_BINARY)) == ERR)
...
PUBLIC void usage()
{
    ...
    errexit(
#ifdef EPOC_TARGET
    " [-03Mimsz[-]] [-T textaddr] [-L libpath] [-llib_extension] [-o outfile] infile...");
#else
