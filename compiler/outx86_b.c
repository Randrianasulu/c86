 ...
 */
static void putamode P3 (const ADDRESS *, ap, ILEN, len, unsigned int, sa)
{
	...
	switch (len) {
	case IL2 + 1:	    /*lint !e408*/  /* type mismatch with switch expression */
	    oprintf (" word ptr ");
	    break;
...
PRIVATE void put_code P1 (const CODE *, ip)
{
    ...
    unsigned int sa = putop (ip->opcode);
...
{
    oprintf ("%s Generated by %s (bas) %s from %s%s",
