...
#define check_expand(OBUF, NEEDED) if((OBUF)->length - ((OBUF)->bufp - (OBUF)->buf) <= (NEEDED)) grow_outbuf ((OBUF), (NEEDED))
...
/* Copy argument list to new array, having prepended values from CPP_OPTS if set */
  ...
  if((opt_ptr = getenv("CPP_OPTS")) != NULL && strlen(opt_ptr) != 0) {
	...
	if ((new_argv[new_argc++] = strtok(opt_cpy, " ")) != NULL) {
	  for (;;) {
		if ((new_argv[new_argc] = strtok(NULL, " ")) == NULL) break;
		new_argv = (char **) xrealloc ((char *) new_argv,
  ...
  new_argv = (char **) xrealloc ((char *) new_argv,
			(size_t) (new_argc + argc) * sizeof(char *));
  {
	int arg_cpy;
...
get_filename:
  ...
  if (*fbeg == '/' || *fbeg == '\\' || *fbeg == ':') {
	...
	fname[flen] = 0;
...
#ifdef QDOS  /* Now do roughly the same for QDOS as for VMS... */
	  {
	  ...
	  assert (stackcheck());
#if 0
	  /* DJW:  No longer need as open uses "Qopen" */
...
void output_line_command (FILE_BUF *ip, FILE_BUF *op, long conditional,
