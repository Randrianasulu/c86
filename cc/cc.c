 ...
 *
 *
 * (c) Copyright 1991-2001 David J. Walker
...
int  qmacflag = 0;			/* QMAC assembler output from c68 required */
#ifdef QL
#ifdef QDOS
int  gwassflag = 0; 		/* GWASS assembler required */
#endif /* QDOS */
#endif /* QL */
 ...
 *-------------------------------------------------------------------------*/
{
	...
	{
		DBG(("ADD_ARGS",0x14,"expanding argv by %d entries (argc=%d, argcmax=%d)",PASS_OPTS_INCR, optptr->argc, optptr->argcmax));
	 ...
	 */
	DBG(("ADD_ARGS",0x18,"&(passes[GWASS_PASS].p_optbuf)=$%x", \
						  &(passes[GWASS_PASS].p_optbuf)));
	if (pass == GWASS_PASS)
	{
		...
		if (optptr->argc > 1)
		{
			...
			oldsize = strlen(*argv);
			optptr->argc = 2;
			DBG(("ADD_ARGS",0x14,"GWASS pass, argc=%d, argv[1]='%s'", \
							optptr->argc, argv));
		}
	}
#endif /* QL */
...
/*=========================================================== COMMAND_LINE */
int 	command_line (CC_PASSES_t * preopts,
					  int			thispass,
					  CC_PASSES_t * postopts,
					  char *		inname,
					  char *		outname)
/*
 ...
 *-------------------------------------------------------------------------*/
{
	...
	DBG(("COMMAND_LINE",0x14,"preopts=$%x, thispass=%d, postopts=$%x", \
								preopts, thispass, postopts));
	DBG(("COMMAND_LINE",0x14,"inname='%s'", \
								inname));
	DBG(("COMMAND_LINE",0x14,"outname='%s'", \
								outname));

	passopts = &passes[thispass];
	DBG(("COMMAND_LINE",0x14,"p_name='%s'", \
								passopts->p_name,pass));
	DBG(("COMMAND_LINE",0x18,"passopts->argv[0]=$%x, value='%s'", \
								passopts->p_optbuf.argv[0], \
								passopts->p_optbuf.argv[0]));
	...
	DBG(("COMMAND_LINE",0x14,"progname=%s", \
								progname));
	DBG(("COMMAND_LINE",0x14,"argv[0]=$%x (%s)", \
								current_pass.argv[0],current_pass.argv[0]));
...
#ifdef LIBDEBUG
	/* Clear error code return fields */
	errno = 0;
	_oserr = 0;
#endif /* LIBDEBUG */
	 ...
	 */
	DBG(("COMMAND_LINE", 0x14, "About to do _spawnvp() call:"));
	 ...
	 */
	DBG(("COMMAND_LINE", 0x14, "About to do execvp() call:"));
	ret = execvp(current_pass.argv[0], (int *)-1L, current_pass.argv);
#else
	/*
	 *	The most portable form is the system() call.
	 */
	DBG(("COMMAND_LINE", 0x14, "about to do the following call\nsystem (\"%s\")", \
										cmdbuf));
	ret = system (cmdbuf);
#endif
	DBG(("COMMAND_LINE", 0x14, " result: ret=%d, errno=%d, _oserr=%d", \
								ret, errno, _oserr));
	free (cmdbuf);
 ...
 *--------------------------------------------------------------------------*/
{
	...
	for( sf = 1; sf < argc; sf++)
	{
		...
		DBG(("PARAMETERS",0x12,"parameter parsing 'sf' loop - argv[sf]='%s'", \
										argv[sf]));
		if( *argv[sf] != '-' )
		{
...
ADD_FILELIST:
			for (ptr = &file_list ; ptr->next ; ptr=ptr->next)
				;
			DBG(("PARAMETERS",0x24,"file_list starts at %p, finishes at %p", \
				  &file_list, ptr));
			ptr->next = malloc (sizeof(struct FILE_LIST));
			DBG (("PARAMETERS",0x24,"Added new FILE_LIST structure at address %p", \
											ptr->next));
...
TRY_GWASS:
				if (CheckCPU() < 0x20)
				{
					printstr ("The '",argvsf,"' option needs the GWASS assembler\n", NULL);
				}
				/* FALLTHRU */
		case ACTION_GWASS:
		...
		case ACTION_TARGET:
				...
				(void) strcpy(workbuf,"-");
				add_option (C68_PASS, strcat(workbuf, argvsf[1] == 'Q'
													   ? &argvsf[2]
													   : &argvsf[1]));
#endif /* QL */
#ifdef QL
#ifdef QDOS
				/*
				 *	For 68020 and higher processors we need to
				 *	use the GWASS assembler on QDOS based sytems.
				 *	(we frig the test by using a strcmp routine)
				 */
				if (strcmp(&workbuf[sizeof("-TARGET=")-1],"68020") >= 0)
				{
					goto TRY_GWASS;
				}
#endif /* QDOS */
...
static
void	Environment_Parameters (char * argv[])
/*		~~~~~~~~~~~~~~~~~~~~~~
 ...
 *--------------------------------------------------------------------------*/
{
	...
	for (ptr = &envtable[0] ; ptr->name != NULL ; ptr++)
	{
		...
		{
			DBG(("ENVIRONMENT_VARIABLES",0x24,"...found, value='%s' stored at 0x%p", \
										result, ptr->result));
		...
		{
#endif /* ENVVAR */
			DBG(("ENVIRONMENT_VARIABLES",0x24,"... not found, default='%s' stored at 0x%p", \
										ptr->deflt, ptr->result));
...
char *argv[];
{
	...
	/*
	 *	Check to see if QMAC assembler format
	 *	output required from C68 phase
	 */
	if (qmacflag)
	{
...
#ifdef QDOS
	/*
	 *	Check to see if we are going to use
	 *	GWASS as the assembler phase and if
	 *	so set up the relevant parameters.
	 */
	if (gwassflag)
	{
...
X_FILE:
		...
		for(pass=start_pass ; (pass <= last_pass) && (pass < LDPRE_PASS) ; pass++)
		{
			...
			if ((0 != alltemp)
			||	(cp->p_suffix != passes[last_pass].p_suffix))
			{
				add_tmp( outname);
			}
			...
			if (GWASS_PASS == pass)
			{
				...
				{
					DBG(("MAIN",0x18,"Copying GWASS object file from '%s' to '%s'", \
										gwname, outname));
